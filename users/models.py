...
class UserManager(BaseUserManager):
    ...
    def create_user(self, username=None, phone_number=None, email=None, password=None, **extra_fields):
        ...
        return self._create_user(username, phone_number, email, password, False, True,  **extra_fields)

    def create_superuser(self, username, phone_number, email, password, **extra_fields):
        return self._create_user(username, phone_number, email, password, True, False, **extra_fields)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(_('username'), max_length=32, unique=True,
                                help_text=_(
                                    'Required. 30 characters or fewer starting with a letter. letters,digits and @/./+/_ only.'),
                                validators=[
                                ...
                                error_messages={
                                    'unique': _("A user with that username already exits. "),
                                }
                                )
    first_name = models.CharField(_('first name'), max_length=30, blank=True)
    last_name = models.CharField(_('last name'), max_length=30, blank=True)
    email = models.CharField(_('email address'), unique=True, null=True, blank=True)
    phone_number = models.CharField(_('mobile number'), unique=True, null=True, blank=True,
                                          validators=[

                                              validators.RegexValidator(r'^989\d{9}',
                                                                        ('Enter a valid mobile number like 989XXXXXXXXX ')
                                                                        )
                                          ],
                                          error_messages={
                                              'unique': _("A user with that username already exits.")
    ...
    is_active = models.BooleanField(_('active'), default=True,
                                   help_text=_('Designates whether the user can log into this admin site.'
                                               'Unselect this instead of deleting acounts'))
